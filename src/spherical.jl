...
function gradient_sphere(f::Field{T}) where {T<:Real}
    grid = f.grid
...
"""
function vorticity_sphere(uE::Field{T}, vN::Field{T}) where {T<:Real}
    grid = uE.grid
    @assert grid === vN.grid
...
"""
function divergence_sphere(uE::Field{T}, vN::Field{T}) where {T<:Real}
    grid = uE.grid
    @assert grid === vN.grid
